    ...
    <div class="wrapper">
        ...
        <van-popup v-model="show" closeable close-icon-position="top-right" position="bottom"
            ...
            <van-form @submit="onSubmit">
                ...
                <van-field v-model="RegisterPassword" :type="isRegisterPassword ? 'password' : 'text'" name="密码"
                    label="密码" placeholder="密码" :rules="[{ required: true, message: '请填写您需要注册的密码' }]" autocomplete="off"
                    :right-icon="isRegisterPassword ? 'closed-eye' : 'eye-o'"
                    @click-right-icon="toggleRegsiterPasswordType" />
            </van-form>
            <div class="register-btn">
                <van-button @click='reg2' style="margin-top:10px" color="#e3e3e3" round block type="info"
                    native-type="submit">
                    注册
                </van-button>
            </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            reg() {
                if (this.LoginUsername === localStorage.getItem("RegisterName") && this.LoginPassword === localStorage
                    .getItem(
                        "RegisterPassword")) {
                    this.$router.push({
                        path: "/mine",
            ...
            reg2() {
                this.$toast('注册成功');
                this.show = false,
                localStorage.setItem("RegisterName", this.RegisterName)
                localStorage.setItem("RegisterPassword", this.RegisterPassword)
            }
        },
